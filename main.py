...
def main(page: ft.Page):
    ...
    def save_Substract(value,substract=None):
        
        if substract == None:
            substract = float(value)
            text.value = "0"
        else:
            substract -= float(value)  
            text.value = f"{substract}"
            print(substract)
        page.update()
    ...
    row4 = ft.Row(controls=[ft.ElevatedButton("0", on_click=lambda e: update_display("0")), ft.ElevatedButton(".", on_click=lambda e: update_display(".")), ft.ElevatedButton("=", on_click=lambda e: calculate())],alignment=ft.MainAxisAlignment.CENTER)
